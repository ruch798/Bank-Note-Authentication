...
def main():
	...
	#using pandas to use as dataframe to take care of null values			
    filename = pd.read_csv('banknote_dirty1.csv',header=None)
    filename = filename.fillna(filename.mean(),inplace=True)	
    #outlier analysis	and hopefully, it's removal    
    #filename.boxplot()	#plotting current outliers of all columns
    col_1 = remove_outlier(filename,2)
    col_1 = remove_outlier(col_1,3)
    col_1.boxplot()
    #col_1.to_csv(r'C:\Users\Administrator\Desktop\Naive-Bayes-Classfier-master\banknote_mean_clean_outlier.csv')				
    #filename.to_csv(r'C:\Users\Administrator\Desktop\Naive-Bayes-Classfier-master\banknote_mean.csv')	
    filename='banknote_mean_clean_outlier.csv'			
